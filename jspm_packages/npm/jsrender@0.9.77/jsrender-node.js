    ...
    for (i = 0; i < l; i++) {
      ...
      if ("" + node === node) {
        ...
        if (tagName === "*") {
          ...
          code += (isGetVal ? (isLinkExpr ? (onError ? "try{\n" : "") + "return " : tagStart) + (useCnvt ? (useCnvt = undefined, useViews = hasCnvt = true, tagRender + (pathBindings ? ((tmplBindings[tmplBindingKey - 1] = tagCtxFn), tmplBindingKey) : "{" + tagCtx + "}") + ")") : tagName === ">" ? (hasEncoder = true, "h(" + params[0] + ")") : (getsVal = true, "((v=" + params[0] + ')!=null?v:"")')) : (hasTag = true, "\n{view:view,tmpl:" + (content ? nestedTmpls.length : "0") + "," + tagCtx + "},"));
