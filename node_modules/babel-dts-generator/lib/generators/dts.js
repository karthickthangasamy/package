...
var generators = {
  ...
  VariableDeclarator: function VariableDeclarator(node, _ref5) {
    ...
    var name = _node$id.name;
    var typeAnnotation = _node$id.typeAnnotation;
  ...
  AssignmentPattern: function AssignmentPattern(node, _ref8) {
    ...
    var _node$left = node.left;
    var name = _node$left.name;
    var typeAnnotation = _node$left.typeAnnotation;

    var type = getTypeAnnotation(typeAnnotation);

    return (0, _ast.createParam)(name, type, { isOptional: true }).fromSource(node);
  },
  ...
  ClassDeclaration: function ClassDeclaration(node, ctx) {
    var shouldExcludeMember = ctx.shouldExcludeMember;
    var ignoreEmptyClasses = ctx.ignoreEmptyClasses;
    var name = node.id.name;
    var superClass = node.superClass;
    var body = node.body;
    var tp = node.typeParameters;
    var stp = node.superTypeParameters;
    var impl = node.implements;
...
function generateNode(meta) {
  ...
  function shouldExcludeMember(memberName) {
    ...
    var memberType = _typeof(meta.ignoreMembers);
...
function getTypeAnnotation(annotated) {
  var defaultType = arguments.length <= 1 || arguments[1] === undefined ? null : arguments[1];
...
function getTypeAnnotationString(annotation) {
  var defaultType = arguments.length <= 1 || arguments[1] === undefined ? 'any' : arguments[1];
